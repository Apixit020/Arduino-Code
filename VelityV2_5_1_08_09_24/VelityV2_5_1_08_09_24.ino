...
void loop() {
  // if (!correctCardScanned) {
  //   rfid_scan();  // Scan the card if not scanned yet
  // } else {
  nexLoop(nex_listen_list);  // ฟังอีเวนต์จากหน้าจอ Nextion

  checkNextionPage();  // Check the current page on Nextion
  ...
  if (onMainPage || correctCardScanned) {
    ...
    // ตรวจสอบการสแกน RFID
    // while (tag != "147225199154" || M !=1 ) // ตรวจสอบว่า UID ตรงกับที่ต้องการหรือไม่
    // {
...
void Gyro() {
  ...
  angleY = 90 - atan2(ax, sqrt(pow(ay, 2) + pow(az, 2))) * 180 / PI;
  ...
  if (angleY > 175) {
    gyro.setPic(60);
  } else if (angleY > 165 && angleY < 175) {
    gyro.setPic(59);
  } else if (angleY > 145 && angleY < 155) {
    gyro.setPic(58);
  } else if (angleY > 135 && angleY < 145) {
    gyro.setPic(57);
  } else if (angleY > 125 && angleY < 135) {
    gyro.setPic(56);
  } else if (angleY > 115 && angleY < 125) {
    gyro.setPic(55);
  } else if (angleY > 105 && angleY < 115) {
    gyro.setPic(54);
  } else if (angleY > 95 && angleY < 105) {
    gyro.setPic(53);
  } else if (angleY > 85 && angleY < 95) {
    gyro.setPic(52);
  } else if (angleY > 75 && angleY < 85) {
    gyro.setPic(51);
  } else if (angleY > 65 && angleY < 75) {
    gyro.setPic(50);
  } else if (angleY > 55 && angleY < 65) {
    gyro.setPic(49);
  } else if (angleY > 45 && angleY < 55) {
    gyro.setPic(48);
  } else if (angleY > 35 && angleY < 45) {
    gyro.setPic(47);
  } else if (angleY > 25 && angleY < 35) {
    gyro.setPic(46);
  } else if (angleY > 15 && angleY < 25) {
    gyro.setPic(45);
  } else {
    gyro.setPic(44);
  }
