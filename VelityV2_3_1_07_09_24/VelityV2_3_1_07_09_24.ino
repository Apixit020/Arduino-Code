...
// NexButton modeToggle2_PARKING = NexButton(5, 6, "ModeToggle2");  // ปุ่มที่หน้า page 5, ID 6
NexNumber rpm1 = NexNumber(4, 14, "RPM");
// NexNumber rpm2_PARKING = NexNumber(5, 17, "RPM");
NexNumber Vbattery1 = NexNumber(4, 17, "Vbattery");
// NexNumber Vbattery2_PARKING = NexNumber(5, 20, "Vbattery");
NexText nexMode1 = NexText(4, 6, "Gears");
// NexText nexMode2_PARKING = NexText(5, 7, "Gears");
NexPage main = NexPage(4, 0, "MAIN");                //65 05 06 01 FF FF FF
// gyro 4,4 "GyroLine"
...
void setup() {
  ...
  nexMode1_RACING.setText("P");

  // ตั้งค่าฟังก์ชัน callback เมื่อปุ่มถูกกด
  modeToggle1_RACING.attachPush(modeToggle1PushCallback);  //รอถามสอบการทำงานส่วนนี้
  // modeToggle2_PARKING.attachPush(modeToggle2PushCallback);
...
void loop() {
  ...
  if (buttonHeld && (millis() - buttonPressTime >= 2000)) {
    ...
    if (!isRacingMode) {
      ...
      isRacingMode = false;
      // main_Page.show();
      // nexMode2_PARKING.setText(currentModeIndex);
    }
...
void modeToggle1PushCallback(void *ptr) {
  // Race = 1;
  // LM = 0;
  Serial.println("ModeToggle1_RACING ถูกกด");
  // if (Race == 1) {
  // เข้าสู่โหมด Racing
  // previousModeIndex = currentModeIndex;
  // currentModeIndex = 5;  // โหมด Racing (สมมติว่าโหมด 5 คือ Racing)
