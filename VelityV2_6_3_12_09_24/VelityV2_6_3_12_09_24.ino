...
// การประกาศ prototype ของฟังก์ชันทั้งหมด
...
void handleRacingMode();
void limitSpeedByAngle();
void calculateAngle();
...
volatile unsigned long hallCount = 0;  // ตัวนับจำนวนครั้งที่ Hall sensor ส่งสัญญาณ
...
NexNumber batper = NexNumber(4, 24, "BattPer");          //เปอร์เซนต์แบต
...
void setup() {
  ...
  pinMode(cut_off, OUTPUT);     // Relay 1 cutoff pin as OUTPUT
  pinMode(front_low, OUTPUT);   // Relay 2 Font Low pin as OUTPUT
  pinMode(front_high, OUTPUT);  // Relay 3 Font High pin as OUTPUT
  pinMode(rear_low, OUTPUT);    // Relay 4 Rear Low pin as OUTPUT
  pinMode(rear_high, OUTPUT);   // Relay 5 Rear High pin as OUTPUT
  ...
  ResetTrip.attachPop(ResetPushCallback);
...
void loop() {
  ...
  if (onMainPage || correctCardScanned) {
    ...
    calculateBattery();
    updateDisplay(val);
    Trip1.setValue(km);  //ส่งค่าระยะทางไปที่จอ km or totalDistance
    if (j == 1) {
...
void calculateRPM() {
  ...
  if (currentMillis - previousMillis >= 833) {  // คำนวณ RPM ทุก 1 วินาที
    noInterrupts();                             // ปิด Interrupt ชั่วคราวเพื่ออ่านค่า pulseCount
    unsigned long currentHallCount = hallCount;
    hallCount = 0;
    interrupts();

    rpm = currentHallCount * 60.0 / PULSES_PER_REVOLUTION;
    int distancePerMinute = rpm * Pi * wheelCircumference / 60;
    totalDistance += distancePerMinute / 60;
    EEPROM.put(address_distance, totalDistance);
    int speedKmh = distancePerMinute * 60 * 3.6;
    ...
    Trip1.setValue(totalDistance);  //ส่งค่าระยะทางไปที่จอ
...
void hallSensorISR() {
  static int lastHallState = digitalRead(RPM_IN);  // เก็บสถานะล่าสุดของ Hall sensor

  int currentHallState = digitalRead(RPM_IN);  // อ่านสถานะปัจจุบันของ Hall sensor

  if (currentHallState != lastHallState) {  // ถ้าสถานะเปลี่ยนแปลง
    if (currentHallState == LOW) {          // และเป็น falling edge
      hallCount++;
      hall++;
    }
    lastHallState = currentHallState;  // อัพเดทสถานะล่าสุด
  }
}
...
void readFromEEPROM() {
  EEPROM.get(address_distance, totalDistance);
...
void calculateBattery() {
  ...
  float voltage = sensorValue * 1.664;  // สมมติว่า ADC ใช้แรงดันอ้างอิง 5V และมี 1023 ระดับ
                                        // คำนวณเปอร์เซ็นต์ตามเงื่อนไข
  if (voltage <= 63.0) {
    percentage = 0;
  } else if (voltage >= 90.0) {
    percentage = 100;
  } else {
    percentage = (voltage - 63.0) * (100.0 / (90.0 - 63.0));
  }
  batper.setValue(percentage);
...
void handleRacingMode() {
  bool buttonPressed = digitalRead(RacingPin) == 1;
...
void limitSpeedByAngle() {
  // Gyro();  // เรียกใช้ฟังก์ชันคำนวณองศา
  if (val > 125) {
    //รอเงื่อนไข
  } else if (val > 115) {
    //รอเงื่อนไข
